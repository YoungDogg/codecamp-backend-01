    ...
    [
      ...
      new Promise((resolve, reject) => {
        storage
          ...
          .pipe(sharp().resize({ width: el.size })) // 4. event 안의 file을 활용하여 썸네일 생성
          .pipe(storage.file(`${el.fname}/${event.name}`).createWriteStream()) // 5. 생성된 썸네일을 재업로드
          .on("finish", () => resolve(`${el.fname}/${event.name}`))
          .on("error", () => reject());
      });
    })
  );

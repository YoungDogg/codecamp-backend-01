...
const createUserDiv = (data) => {
  ...
  // 이전 버전에서 마스킹 안된 정보를 고려하여 한 번 더 마스킹 한다
  personalItem.textContent = data?.personal.substring(0, 6) + "-*******" || '220111-*******'
  ...
  preferItem.textContent = data?.favoriteSite || 'https://codebootcamp.co.kr'
